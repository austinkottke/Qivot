...
| [`dashboard`](dashboard) | 📖 **Tutorial** — a sales-analytics dashboard: a **window-function leaderboard** (`qiRawQuery` with a CTE + `RANK()`/running-`SUM()` → typed rows) and an **async, cancellable "Recompute"** (`QiAsync` + `QiConnectionPool` + `QiCancelToken`) with a live progress bar. → [walkthrough](dashboard/README.md) |
