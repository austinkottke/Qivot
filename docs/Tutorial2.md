...
The QiModel::load() support to load a single record from database and store to the the current QiModel() instance. 

The function take an argument with type of QiWhere , which can represent the filter rules. 
...
The rule stored in QiWhere can be more complex. Please refer to the API document on its advanced usage.

=== QiQuery ===

QiQuery is a class to query record from database. Unlike QiModel::load(), it can query for multiple record , and it provides more advanced interface and filter rules.
...
int main(int argc, char *argv[])
{
    ...
    query = query.filter(QiWhere("karma") > 50); // Contruct a query object to find user where karma > 50;
    ...
    list = query.filter(QiWhere("karma") > 50  ).all();
    qDebug() << list;

    // Get the list of user where "karam > 50 and karma <  80"
    list = query.filter(QiWhere("karma") > 50 && QiWhere("karma") < 80).all();
    qDebug() << list;

    // Get the list of user where "karam >= 50 and karma <= 80"
    list = query.filter(QiWhere("karma").between(50,80)).all();
    qDebug() << list;

    // Count no. of record .
    int count = query.filter(QiWhere("karma") > 50).count();
