...
To declare a model with foreign key , you just need to use QiForeignKey field type
...
Moreover, QiForeignKey support an auto loading mechanism. You may access the record "linked" by the QiForeignKey through its extended interface (using -> operator).

By default, QiForeignKey only store the foreign key value.  Whatever the user request to access the "linked" record field, it will be loaded automatically.
...
public:
    ...
    virtual QiSharedList initialData();
};
...
int main(int argc, char *argv[])
{
    ...
    User tester1;
    tester1.load(QiWhere("userId = ", "tester1"));

    User tester2;
    tester2.load(QiWhere("userId = ", "tester2"));

    User tester3;
    tester3.load(QiWhere("userId = ", "tester3"));
    ...
    QiList<FriendShip> list = FriendShip::objects()
                              .filter(QiWhere("a = ", tester1.id )).all();
